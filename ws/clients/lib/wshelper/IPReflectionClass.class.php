...
class IPReflectionClass extends reflectionClass {
	 ...
	 */
	public function getMethods($alsoProtected = true, $alsoPrivate =false, $alsoHerited = true){
		$ar = parent::getMethods();
		foreach($ar as $method){
			if($method->name == '__construct')
				continue;	
			if($method->name == 'exception_handler')
				continue;		
					
			if((!$method->isPrivate() || $alsoPrivate) && 
			   (!$method->isProtected() || $alsoProtected) && 
			   (($method->getDeclaringClass()->name == $this->classname) || $alsoHerited)) {
					//PP only do extra relection is that method interest us
