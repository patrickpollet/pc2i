...
class IPXMLSchema {
	...
	public function addComplexType($type, $name = false, $parent = false) {
        //echo 'ACT name='.$name .' type='.$type.' '.$parent. '<br>';
	...
	public function addTypeElement($type, $name, $parent, $optional = false) {
        //  echo 'ATE name='.$name .' type='.$type. '<br>';
...
//print ('['.$type.']<br/>');
	...
	public function isComplexType($type) {
		//XML Schema types
		if(isset($this->types[$type])) return true;
		else return false;
	}
	...
	public function isArrayType($type) {
		...
		return (substr($type,-5)==='Array') || (substr($type,-2)==='[]');
		//else return false;
	}
	...
	public function getBaseType($type) {
		...
		if ($this->isArrayType($type))
		   return str_replace(Array("Array","[]"), Array("", ""),$type);
		else return '';
	}
