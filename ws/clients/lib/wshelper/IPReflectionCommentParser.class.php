...
class IPReflectionCommentParser{
	...
	function parse() {
		...
		$this->smallDescriptionDone = false;
	...
	function parseDescription($descriptionLine) {
        $descriptionLine=trim($descriptionLine); //PP remove the leading star after trimming the line ...
        $descriptionLine=str_replace(Array("\$","\"","'"), Array("", "",""),$descriptionLine); // remove unwanted chars 
	...
	function parseTagLine($tagLine) {
		...
		switch(strtolower($tag)){
			...
			case 'global':
				...
				break;
			case 'param':
			    $pname= str_replace('$', '', trim($tagArr[2]));
				try { 
				$o=new IPReflectionParameter(array($this->obj->class,$this->obj->name),$pname);
				
				$o->type = trim($tagArr[1]);
				$o->comment = addslashes(implode(" ",array_slice($tagArr,3))); //PP
				$this->obj->parameters[$o->name] = $o;
				} catch (Exception $ex) {
					print " ERROR parameter CLASSE= $this->obj->class \n METHODE=$this->obj->name \n PARAM=$pname";
					die();
				}
				break;
			case 'return':
				$this->obj->return = trim($tagArr[1]);
				$this->obj->returndesc = addslashes(implode(" ",array_slice($tagArr,2))); //PP
				 break;
			case 'link':break;
			case 'see':break;
			case 'since':
			...
			case 'var':
				...
				$comment=addslashes(implode(" ",$tagArr));
