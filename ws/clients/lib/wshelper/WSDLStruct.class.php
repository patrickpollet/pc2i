...
class WSDLStruct {
		...
		foreach((array)$this->services as $serviceName => $service){
			...
			foreach((array)$service->methods as $operation){
				...
				if(count($parameters)>0 || self::CREATE_EMPTY_INPUTS){
					...
					$this->addMessage($messageName, $para,true);
				...
				if(strtolower(trim($operation->return))!='void'){
					...
					$this->addMessage($messageName,Array($operation->name."Return" => $operation->return),false);
				}
			}
	 ...
	 * @return domElement
	 */
	protected function addOperation($operationName, $serviceName){
	 ...
	 * @return domElement
	 */
	protected function addService($serviceName){
	 ...
	 * @return domElement
	 */
	protected function addPortType($serviceName){
	 ...
	 * @return domElement
	 */
	protected function addBinding($serviceName){
	 ...
	 * @param boolean $inout 
	 * @return the created Dom Element (Pp for debug purpose)
	 */
	protected function addMessage($name, $parts,$inout){
	 ...
	 */
	protected function addInput($parent, $input=true){
	 ...
	 */
	protected function addOutput($parent){
	...
	protected function addToDebug($msg){
	 ...
	 * @return domNode
	 */
	protected function addElement($name, $parent=false, $ns=false){
		if($ns)
