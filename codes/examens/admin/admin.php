...
$nbPassages=compte_passages($idq,$ide); //inclus les r�sultats non enregistr�s
...
if ($doit) {
	...
	switch ($action) {
		...
		case 'sim' :
			simule_passage($idq,$ide); //pools g�r�s dans cette fonction
			 espion3("simulation_passage","examen",$ide.".".$idq,$ligne);
		break;
		case 'asim' :
			annule_simulation($idq,$ide); //pools g�r�s dans cette fonction
		...
		case 'sinp' :
    		...
    		foreach ($inscrits as $i) {
            		if (compte_passages($idq, $ide, $i->login)==0) {
                        ...
                        // rev 978 m�nage des anonymes ne l'ayant pas termin�s
