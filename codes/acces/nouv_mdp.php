...
if ($mel) {
    ...
    // on passe AUSSI un errMsg1 non vide pour ne pas d�voiler l'erreur MySql !  rev 897
	...
	if (!$ligne || empty($ligne->email)) {
		...
		//if (is_admin($ligne->login,$ligne->etablissement)) {NON ! $USER pas renseign� !
        // rev 894-896 on n'envoie pas le passe d'un admin meme � lui !
        if ($ligne->est_admin_univ=="O" || $ligne->est_superadmin=="O") {
			...
			$nouveau_passe = mot_de_passe_a(7); // g�n�re un nouveau mot de passe de 7 caract�res
			$futur_verif = md5(uniqid(rand())); // cl� de v�rification
			$rec=new StdClass();
	...
	$tpl->assignInclude("contenu",$forme,T_BYVAR);  // le template g�rant la configuration
