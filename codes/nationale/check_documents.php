   ...
   // print "question {$q->id_etab}.{$q->id}:\n";
    ...
    foreach ($docs as $d) {
        if ($b64=@encode_document($d->id_doc.'.'.$d->extension, $q->id,$q->id_etab )) {
            echo "question {$q->id_etab}.{$q->id} �tat : {$q->etat} a le document {$d->id_doc}.{$d->extension} OK <br/>\n";
        }
        else
          echo "question {$q->id_etab}.{$q->id} CERT = {$q->certification} POS = {$q->positionnement}  �tat : {$q->etat} N'A PLUS LE DOCUMENT {$d->id_doc}.{$d->extension} <br/>\n";
...
foreach ($questions as $q) {
    ...
    if (empty($reps)) {
        ...
        print $q->etat.' ';
        print  ("KO question sans AUCUNE r�ponse <br/>\n");
    }
    else {
        $nbr=count($reps);
        foreach ($reps as $rep) {
        ...
        if ($nb==0) {
            ...
            print "CERT = {$q->certification} POS = {$q->positionnement} : ";
            print $q->etat.' ';
            print  ("KO question ($nbr r�ponses) sans BONNE r�ponse <br/>\n");
        }
    }
    // else print ("OK\n");
