...
class TemplateParser
{
  ...
  var $version;
   ...
   function __prepare()
   {
    ...
    /**
     * TemplateParser::__prepareTemplate()
     * 
     * @param $tpl_file
     * @param $type
     * @return 
    ...
    function __prepareTemplate( $tpl_file, $type )
    {
        ...
        else
        {
            $this->{$tplvar}["content"] = @file( $tpl_file ) or
                die( $this->__errorAlert('Template Error: Couldn\'t open [ '. $tpl_file .' ]!'));
        }
    ...
    /**
     * TemplateParser::__parseTemplate()
     * 
     * @param $tplvar
     * @param $blockname
     * @param $initdev
                           ...
                           //check if the includescript file is assigned by the assignInclude function
                            ...
                            if( $include_defined )
                            {
								...
								if( $type == T_BYFILE )
								{
                                    ...
                                    {
                                        $this->__errorAlert( 'Template Error: Couldn\'t include script [ '. $include_file .' ]!' );
										exit();
                                    }
								}
								else
                           ...
                           //do match for 'AS'
                            if (preg_match('/(.+) AS (.+)/', $regs[2], $reuse_regs))
                            {
                               ...
                               //test if original block exist
                                ...
                                {
                                    $this->__errorAlert('Template Error: Can\'t find block \''. $originalbname .'\' to REUSE as \''. $copybname .'\'');
                                }
                            }
                            else
                            {
...
class Template extends TemplateParser
{
  ...
  var $prepared;
	 ...
	 * @access public
     */
    function Template( $tpl_file='', $type= T_BYFILE )
    {
    ...
    function __deSerializeTPL( $stpl_file, $type )
    {
        if( $type == T_BYFILE )
        {
            $serializedTPL = @file( $stpl_file ) or
                die( $this->__errorAlert('Template Error: Can\'t open [ '. $stpl_file .' ]!'));
        }
        else
        {
  ...
  /**********
    ...
    function affichageComplet()
    {
        ...
        {
            $this->__errorAlert('Template Error: Template isn\'t prepared!');
        }
    }

    /**
     * Template::getOutputContent()
     * 
     * @return 
	 * 
	 * @access public
