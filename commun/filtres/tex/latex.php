<?php // $Id: latex.php,v 1.9.2.2 2010/04/10 00:17:45 iarenaza Exp $
    ...
    class latex {
        ...
        function latex() {
            ...
            $this->temp_dir = $CFG->chemin_ressources . "/tmp/latex";
            /**
...
//                return false;
            ...
            /**
             * IMPORTANT PP
             * sur une ubuntu avec un imagemagick 6.5.8 recent il faut utiliser l'argument -flatten
             * pour avoir des images gif DECENTES, mais alors plus de transparence ...
             * donc par défaut le format est PNG ...
             * http://www.imagemagick.org/discourse-server/viewtopic.php?f=3&t=14368
             */
            if ($CFG->filter_tex_convertformat=='gif' && $CFG->filter_tex_utiliser_flatten_pour_gif)
                $flatten='-flatten';
            else $flatten='';
            ...
            $command = "{$CFG->filter_tex_pathconvert} -density $density -trim $flatten  $bg_opt $ps $img";
            if ($this->execute($command, $log )) {
                return false;
            }
