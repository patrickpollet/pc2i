...
function string_file_picture_tex($imagefile, $tex= "", $height="", $width="", $align="middle", $alt='') {
   ...
   */
    ...
    if ($imagefile) {
        /*
        if (!file_exists("$CFG->chemin_ressources/filter/tex/$imagefile") && has_capability('moodle/site:config', get_context_instance(CONTEXT_SYSTEM))) {
          $output .= "<a href=\"$CFG->chemin_commun/filtres/tex/texdebug.php\">";
        } else {
        ...
        */
        ...
        if (!empty($CFG->slasharguments)) {        // Use this method if possible for better caching
            $output .= "$CFG->wwwroot/filter/tex/pix.php/$imagefile";
        } else {
            $output .= "$CFG->wwwroot/commun/filtres/tex/pix.php?file=$imagefile";
        }
        $output .= "\" $style/>";
       // $output .= "</a>";
    } else {
