...
/*
 * bibliotheque de manipulations des l'entit�s utilisateurs, inscrits et etablissement
 */
 ...
 * ne sont pas encore connu ... (c'est elle qui les lit !!!) donc is_adm�in() renvoie FAUX
 */
  // NE RIEN FAIRE ICI
 }


// context definitions  non utilis�es en V 1.5
...
* @todo si pas loggué aucune capacité sauf le positionnement anonyme
*/
...
/**
 * teste une capacit� dans un contexte
 * pour l'instant le contexte est un �tablissement
 * TODO gerer les composantes par h�ritage
 */

function a_capacite( $capa,$context=false, $id=false) {
	global $USER;
	if (!$id) $id=$USER->id_user;
	if (!$id)  return false; //pas log� rien

	if (! $context) $context=$USER->id_etab_perso;
	if (! $context)  return false;
    if (is_admin($id,$context)) return true;  //pour cet etablissement et ses composantes
    if ($context !=$USER->id_etab_perso) return false;  // pas d'h�ritage encore sur les autres droits
 ...
 * a lire une fois a chaque entr�e sur une page
 *
 */

function lecture_droits ($context=false) {
	global $USER,$CFG;
	// global $row_droits,$row_admin; //compatibilt� V <1.41 en attendant gros m�nage des tests des droits dans les pages FINI le 20/04/2009 svn 714
...
//v�rifie qu'un droit (ex. "ql","em")  est bien dans la tableau global des
// droit de l'utilisateur courant  version 1.4

//define ('DEBUG_DROITS',1);
function teste_droit( $droit) {
    global $ide,$idexe; //l'etablissement du "machin" concern�" relu par required_param (pas terrible !)
    global $USER,$CFG;

    //manipulation des questions : $idexe a priorit� sur $ide !
    //$ide n'est pas toujours renseign� !
    if (!isset($ide))
        $ide=$USER->id_etab_perso;

    //etablissement vis� peut etre different (exemple une question a valider
    // il FAUT corriger ca ....)
    if (isset($idexe)) $etab=$idexe;
    else $etab=$ide;
    if (!isset($USER->droits)) return false; // un �tudiant ...
...
// v�rifie droit or die !
// � inserer au d�but de chaque page et fichier inclus
...
/**
 * renvoie les m�thodes d'authentification dans un tableau d'objets
 * pr�t � l'emploi dans un select
 * en ajouter si n�cessaire et adapter la fonction lib_auth.php:authentifie_compte($compte,$passe)
 * rev 936 on teste si LDAP est pertinent pour cet �tablissement
...
/**
 * rev 1022 type d'annauires LDAP support�
 */
...
/**
 * rev 944  simplificationb et appel par le WS (ex profs ayant pass�s un examen possible)
...
function cree_candidat ($ligne, $ide=false) {
    ...
    //rev 921 valeurs par d�faut pour appel par le web service
    ...
    //attention la table c2iinscrits n'a pas de num�ro automatique
    //donc si on demande returnid (3eme param�tre) ca renvoie 0 false !!!!
...
function supprime_candidat ($supp_id) {
	  ...
	  if (est_inscrit_examen(false,false,$supp_id)==0  || is_utilisateur_anonyme($supp_id)) {
            //ses notes
            //TODO selon un CFG garder ses r�sultats pour les stats examens mais pas son historique !
            ...
            require_once($CFG->chemin_commun."/lib_resultats.php");  // non charg�e par c2i_params
			foreach($exams as $exam) {
			   purge_resultats_inscrit($exam->id_examen,$exam->id_etab,$supp_id,$avecHistorique=true);
			}
            //ses parcours
			if ($CFG->utiliser_notions_parcours) { // rev 984 attention si lib_nions a �t� charg�e
...
/**
 * @access private
 * retourne la concat�nation du nom et du pr�nom de la personne ayant pour login $login
 * selon les r�gles de configuration
 */
...
/**
 * essaie d'appliquer la r�gle nom pr�nom a un nom complet
 * d'auteur tel qu'�crit dans les tables questions et examens
 */

function applique_regle_nom_prenom($auteur){ // rev 841

    $tmp=explode(" ",$auteur);
    if (count($tmp) !=2 ) return $auteur; //rat� ou nom compos� ex Bruno Le Berre
...
/**
 * le parametre connexion est rest� pour la comat V 1.4
 * il n'est pas utilis�'
 */
...
/**
 * tant que les deux tables c2iinscrits et c�iutilisataurs n'ont pas �t� fusionn�es
 * passer par ici
...
/**
 * tant que les deux tables c2iinscrits et c2iutilisateurs n'ont pas �t� fusionn�es
 * passer par ici
 *  * TODo passer par get_xxxx et virer le global connexion
 */
...
function supprime_profil ($supp_id) {
	 // double v�rification que le profil n'est pas utilis�
 ...
 * est appel� par la fiche d'un profil et pour rappeler les profils connus (ajout personnel)'
 * completer sin on ajoute des capacit�s a un profil
 */
...
/**
 * coche ou non les cases du profil en table renvoy� ci-dessus
 * completer sin on ajoute des capacit�s a un profil
 */
function garni_table_profil($tpl,$ligne) {
	//si on avait le bonne id�e de mettre les meme noms aux balises
 ...
 * donc ne pas appeler deux fois pour un m�me compte !
 */
function no_anonyme(){
	global $CFG;
...
/**
 * cree un compte anonyme dans l'�tablissement $ide
 * @param $ide l'�tablissement de ratachment du compte (normalement celui de l'examen associ�)
 * @param $email adresse de courriel �ventuelle
...
//revision 962 simplification des �ditions des profils
