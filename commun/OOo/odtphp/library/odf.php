 ...
 * @version 1.3
 * 
 * Modifié PP charset par défaut est UTF8 en V2 
 */
class Odf
{
    protected $config = array(
    	...
    	'DELIMITER_LEFT' => '{',
    	'DELIMITER_RIGHT' => '}',
		'PATH_TO_TMP' => null    //system default
   	);
    protected $file;
    ...
    public function setVars($key, $value, $encode = true, $charset = 'UTF-8')
    {
        if (strpos($this->contentXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) === false) {
           //TODO ne pas raler si la balise n'existe pas, juste ignorer ...
    ...
    public function setImage($key, $value)
    {
        ...
        $xml = <<<IMG
<text:span text:style-name="T1"><draw:frame draw:style-name="fr1" draw:name="$filename" text:anchor-type="as-char" svg:width="{$width}cm" svg:height="{$height}cm" draw:z-index="3"><draw:image xlink:href="Pictures/$file" xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"/></draw:frame></text:span>
IMG;
