 ...
 */
 ...
 /**
  * adpat� � la PF C2I v 1.5 PP Octobre 2009
  */
...
class MoodleODSWorkbook {
    ...
    function MoodleODSWorkbook($filename) {
    	...
    	$this->filename = $filename;
    	 $this->utf8encode=$CFG->encodage !='UTF-8';
    }
    ...
    /// Create the Moodle Worksheet. Returns one pointer to it
     	$name=$this->utf8encode ?utf8_encode( $name):$name;
    ...
    function close() {
        global $CFG;
        require_once($CFG->chemin_commun.'/lib_fichiers.php');

        $dir = $CFG->chemin_ressources.'/tmp/ods/';
        cree_dossier_si_absent($dir);
        $dir .= time();
        cree_dossier_si_absent($dir);
        cree_dossier_si_absent($dir.'/META-INF', false);
       ...
       // $files[] = "$dir/content.xml";
        ...
        $filename = $CFG->chemin_ressources.'/tmp/ods/'.$this->filename;
       // print($filename);
        /**
        zip_files($files, $filename);
        **/
        zip_dossier($dir,'.',$filename);
...
class MoodleODSWorksheet {
    ...
    function write_string($row, $col, $str, $format=0) {
        ...
        $this->data[$row][$col]->value = $this->utf8encode ?utf8_encode( $str):$str;
...
<office:document-meta xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" xmlns:meta="urn:oasis:names:tc:opendocument:xmlns:meta:1.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xlink="http://www.w3.org/1999/xlink">
 <office:meta>
  ...
  <meta:initial-creator>'.get_fullname($USER->id_user).'</meta:initial-creator>
  <meta:editing-cycles>1</meta:editing-cycles>
  <meta:creation-date>'.strftime('%Y-%m-%dT%H:%M:%S').'</meta:creation-date>
  <dc:date>'.strftime('%Y-%m-%dT%H:%M:%S').'</dc:date>
  <dc:creator>'.get_fullname($USER->id_user).'</dc:creator>
 </office:meta>
