...
<?php
  ...
  // Parameters :
  // --------------------------------------------------------------------------------
  function PclTraceActionOld($p_entry)
  {
    global $g_pcl_trace_level;
    global $g_pcl_trace_mode;
    global $g_pcl_trace_filename;
    ...
    //{
      for ($i=0; $i<$p_entry['index']; $i++)
        echo "---";
      if ($p_entry['type'] == 1)
        echo "<b>".$p_entry['name']."</b>(".$p_entry['param'].") : ".$p_entry['message']." [".$p_entry['file'].", ".$p_entry['line']."]\n";
      else if ($p_entry['type'] == 2)
        echo "<b>".$p_entry['name']."</b>()=".$p_entry['param']." : ".$p_entry['message']." [".$p_entry['file'].", ".$p_entry['line']."]\n";
      else
        echo $p_entry['message']." [".$p_entry['file'].", ".$p_entry['line']."]\n";
    //}
  }
  // --------------------------------------------------------------------------------
 ...
 // --------------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------------
  // Function : PclTraceAction()
  // Description :  modifié PP pour support fichier log
  // Parameters :
  // --------------------------------------------------------------------------------
  function PclTraceAction($p_entry)
  {
    ...
    global $g_pcl_trace_entries;
      ...
      ob_start();
      PclTraceActionOld($p_entry);
      $content = ob_get_contents();
      ob_end_clean();
      if ($g_pcl_trace_mode == "normal")   echo nl2br($content);
      if ($g_pcl_trace_mode == "log") {
           $fp=fopen($g_pcl_trace_filename,"a");
           if ($fp) {
                fwrite($fp,$content."\n");
                fclose($fp);
           }else print ("bug :".$g_pcl_trace_filename."<br/>");
      }
