...
define('NO_HEADERS',1); // pas d'entete par c2i_params  TRES IMPORTANT
...
$chemin="../../..";
    ...
    require_once($chemin_commun."/c2i_params.php");
     ...
     $id= required_param('id', PARAM_INT); //chemin vers le dossier des documents
    ...
    header("Content-type: application/x-javascript; charset: {$CFG->encodage}");  // Correct MIME type
    header("Last-Modified: " . gmdate("D, d M Y H:i:s", $lastmodified) . " GMT");
    header("Expires: " . gmdate("D, d M Y H:i:s", time() + $lifetime) . " GMT");
    header("Cache-control: max_age = $lifetime");
    header("Pragma: ");

    $lang ='fr'; // current_language();
...
*/
    ...
    // $url = $locale_url_univ.'/commun/editeurs/htmlarea/';
    ...
    this.toolbar = [
        ...
        [ "justifyleft", "justifycenter", "justifyright", "justifyfull", "separator",
         // "lefttoright", "righttoleft", "separator",
          ...
          //"createanchor",
          "createlink", "unlink", "nolink", "separator",
          "insertimage", "inserttable",
          // "insertsmile",
          "insertchar", "search_replace",
...
HTMLArea.prototype._createToolbar = function () {
    ...
    function createSelect(txt) {
        ...
        switch (txt) {
            ...
            if (typeof dropdown != "undefined") {
                ...
                alert("ERROR PP [createSelect]:\nCan't find the requested dropdown definition "+txt);
            }
            break;
        }
        if (options) {
    ...
    function createButton(txt) {
        ...
        if (el) {
            ...
            alert("PP FIXME: Unknown toolbar item: " + txt);
        }
        return el;
    };
...
HTMLArea.prototype._insertImage = function(image) {
    ...
    if(!empty($id)) { //and has_capability('moodle/course:managefiles', get_context_instance(CONTEXT_COURSE, $id))) {
        echo "insert_image.php?ide=$ide&id=$id";
    } else {
        echo "insert_image_std.php?ide=$ide&id=$id";
    }
...
HTMLArea.prototype._insertTable = function() {
    ...
    var range = this._createRange(sel);
    var editor = this;  // for nested functions
    this._popupDialog("<?php echo p_session("insert_table.php?id=$id&ide=$ide"); ?>", function(param) {
        if (!param) {   // user must have pressed Cancel
            return false;
        }
        var doc = editor._doc;
...
HTMLArea.checkSupportedBrowser = function() {
    ...
    if (HTMLArea.is_gecko) {
        ...
        if (navigator.productSub < 20030210) {
            alert("PP Mozilla < 1.3 Beta is not supported!\n" +
